...
### AmneziaWG
Маскирующийся вариант WireGuard. Он добавляет мусорные пакеты и рандомизированные заголовки рукопожатия (параметры `Jc`, `Jmin`, `Jmax`, `S1`–`S4`, `H1`–`H4`, `I1`–`I5`), благодаря чему DPI-системы, которые обнаруживают и блокируют обычный WireGuard, перестают распознавать трафик. Задайте параметры маскировки в соответствии с вашим сервером AmneziaWG (или эндпоинтом Cloudflare WARP на AmneziaWG). Та же быстрая криптография Curve25519 / ChaCha20-Poly1305, что и в WireGuard, но пакеты больше не выглядят как WireGuard.

**Параметры Amnezia 3.1** (требуют sing-box-extended **≥ v1.14.0-extended-2.7.0**):

- **Ключ защиты заголовков** — base64-ключ на 32 байта для ChaCha20-защиты заголовков пакетов. При его включении используйте совместимые значения `H1`–`H4` = `1/2/3/4` и держите `S1`–`S4` ≥ 12.
- **Дополнительное паддинг-заполнение** — дополнительное заполнение полезной нагрузки, одно значение или диапазон (напр. `50-100`).
- **Времена rekey / Reject after time / Keepalive / лимит попыток рукопожатия** — параметры таймингов в секундах, каждое — одно значение или диапазон (напр. `100-140`, `4-6`, `160-200`, `8-12`, `15-20`). Они рандомизируют тайминги, противодействуя статистическому анализу трафика.
- **Случайные хвосты пакетов / Отключить cookies** — логические параметры Amnezia 3.1, зеркалящие `RandomTrailers` и `DisableCookies` сервера. «Хвосты» дописывают случайные байты в конец handshake-пакетов и позволяют принимать такие пакеты; без этой опции клиент молча отбрасывает ответы сервера с хвостом — рукопожатие не проходит. Требуется ядро с поддержкой `random_trailers` (sing-box-extended с PR [#155](https://github.com/shtorm-7/sing-box-extended/pull/155)).

Импорт `vpn://` и кнопка **Импорт WG (.conf) файла** заполняют все эти поля автоматически — см. [Подписки](Subscriptions-ru).
