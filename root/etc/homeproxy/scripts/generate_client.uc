...
function generate_endpoint(node) {
	...
	const endpoint = {
		...
		amnezia: (node.type === 'amneziawg') ? {
			...
			/* Amnezia 3.x (requires sing-box-extended >= v1.14.0-extended-2.7.0) */
			header_protection_key: node.amnezia_header_protection_key || null,
			content_padding_addition: node.amnezia_content_padding_addition || null,
			rekey_after_time: node.amnezia_rekey_after_time || null,
			rekey_timeout: node.amnezia_rekey_timeout || null,
